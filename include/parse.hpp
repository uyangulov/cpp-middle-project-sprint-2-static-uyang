...
consteval auto get_current_source_for_parsing()
{
    ...
    constexpr auto src_start = [&]
    {
        ...
        else
        {
            ...
            return pos != std::string_view::npos ? pos + sep.size()
                                                 : src_sv.size();
        }
    }();
    ...
    constexpr auto src_end = [&]
    {
        ...
        constexpr auto sep = fmt_sv.substr(
            fmt_end + 1, (I < fmt.number_placeholders - 1)
...
consteval std::expected<T, parse_error> parse_input()
{
    ...
    constexpr fixed_string<len_i - 1> str_i{fmt.fmt.data + first_i + 1,
                                            fmt.fmt.data + second_i};
